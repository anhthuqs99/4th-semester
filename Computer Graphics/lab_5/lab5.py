...
class Kg5App(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        tk.Tk.title(self, "Lab_5")
        tk.Tk.geometry(self, str(can_x + options_size) + "x" + str(can_y))

        # self.image = QImage(can_x, can_y, QImage.Format_ARGB32_Premultiplied)
        self.image = Image.new("RGB",(can_x, can_y), "#ffffff")
...
def fill(self):
    ...
    if len(self.edges) < 3:
        ...
        return -1
    x_max = find_max_x(self.edges)
    # print(x_max)
    intersections = get_intersections(self.edges)
    draw_image(self, intersections)
    #self.image.show()
    if int(self.delay_var.get()) == 1:
        ...
        fill_figure_delay(self, intersections, x_max)
    else:
        fill_figure(self, intersections, x_max)
