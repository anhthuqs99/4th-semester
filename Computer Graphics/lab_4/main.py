...
class GraphPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        res = time_test(100000)

        f = Figure(figsize=(5, 5), dpi=100)

        plt = f.add_subplot(111)

        plt.plot(res[5], res[0], label="Каноническое уравнение")
        plt.plot(res[5], res[1], label="Параметрическое уравнение")
        plt.plot(res[5], res[2], label="Алгоритм Брезенхема")
        plt.plot(res[5], res[3], label="Метод средней точки")
        plt.plot(res[5], res[4], label ="Библиотечной метод")
